				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"capacity": {
							Type:         schema.TypeInt,
							Required:     true,
							ValidateFunc: validate.ClusterCapacity,
						},
					},
				},
			},
...
func resourceStreamAnalyticsClusterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	client := meta.(*clients.Client).StreamAnalytics.ClusterClient
	...
	if err != nil {
		if !utils.ResponseWasNotFound(existing.Response) {
			return fmt.Errorf("checking for present of existing Stream Analytics Cluster %q (Resource Group %q): %+v", name, resourceGroup, err)
		}
	}
	if !utils.ResponseWasNotFound(existing.Response) {
	...
	future, err := client.CreateOrUpdate(ctx, props, resourceGroup, name, "", "*")
	if err != nil {
		return fmt.Errorf("creating Stream Analytics Cluster %q (Resource Group %q): %+v", name, resourceGroup, err)
	}

	if err := future.WaitForCompletionRef(ctx, client.Client); err != nil {
		return fmt.Errorf("waiting for the creation of the Stream Analytics Cluster %q (Resource Group %q): %+v", name, resourceGroup, err)
	}
...
func resourceStreamAnalyticsClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("updating Stream Analytics Cluster %q (Resource Group %q): %+v", id.Name, id.ResourceGroup, err)
	}

	if err := future.WaitForCompletionRef(ctx, client.Client); err != nil {
		return fmt.Errorf("waiting for the update of the Stream Analytics Cluster %q (Resource Group %q): %+v", id.Name, id.ResourceGroup, err)
	}
	return resourceStreamAnalyticsClusterRead(d, meta)
}
...
func resourceStreamAnalyticsClusterDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err := future.WaitForCompletionRef(ctx, client.Client); err != nil {
		return fmt.Errorf("waiting for the deletion of the Stream Analytics Cluster %q (Resource Group %q): %+v", id.Name, id.ResourceGroup, err)
	}
	return nil
}
