...
func resourceResourceMoverMoveResourceVirtualMachine() *schema.Resource {
	return &schema.Resource{
		Create: resourceResourceMoverMoveResourceVirtualMachineCreateUpdate,
		Read:   resourceResourceMoverMoveResourceVirtualMachineRead,
		Update: resourceResourceMoverMoveResourceVirtualMachineCreateUpdate,
		Delete: resourceResourceMoverMoveResourceVirtualMachineDelete,
					...
					Schema: map[string]*schema.Schema{
						...
						"target_availability_zone": {
							...
							ValidateFunc: validation.StringInSlice([]string{
								string(resourcemover.One),
								string(resourcemover.Two),
								string(resourcemover.Three),
								string(resourcemover.NA),
							}, false),
						},
...
func resourceResourceMoverMoveResourceVirtualMachineCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if !utils.ResponseWasNotFound(existing.Response) {
			return tf.ImportAsExistsError("azurerm_resource_mover_move_resource_virtual_machine", id)
		}
	}

	properties := resourcemover.MoveResource{
		Properties: &resourcemover.MoveResourceProperties{
			ResourceSettings:   expandArmMoveResourceVirtualMachineResourceSettings(d.Get("resource_setting").([]interface{})),
	...
	return resourceResourceMoverMoveResourceVirtualMachineRead(d, meta)
}

func resourceResourceMoverMoveResourceVirtualMachineRead(d *schema.ResourceData, meta interface{}) error {
	subscriptionId := meta.(*clients.Client).Account.SubscriptionId
	client := meta.(*clients.Client).ResourceMover.MoveResourceClient
	...
	if props := resp.Properties; props != nil {
		vmSetting, ok := props.ResourceSettings.AsVirtualMachineResourceSettings()
		if !ok {
			return fmt.Errorf("resource Mover Move Resource %q is not type `azurerm_resource_mover_move_resource_virtual_machine`", d.Id())
		}

		if err := d.Set("resource_setting", flattenArmMoveResourceVirtualMachineResourceSettings(vmSetting)); err != nil {
...
func resourceResourceMoverMoveResourceVirtualMachineDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).ResourceMover.MoveResourceClient
...
func expandArmMoveResourceVirtualMachineResourceSettings(input []interface{}) resourcemover.BasicResourceSettings {
	if len(input) == 0 {
		return nil
	}
	v := input[0].(map[string]interface{})
	res := resourcemover.VirtualMachineResourceSettings{
		ResourceType:       resourcemover.ResourceTypeMicrosoftComputevirtualMachines,
		TargetResourceName: utils.String(v["target_resource_name"].(string)),
		TargetVMSize:       utils.String(v["target_vm_size"].(string)),
	}

	if setId, ok := v["target_availability_set_id"].(string); ok && setId != "" {
		res.TargetAvailabilitySetID = utils.String(setId)
	}

	if zone, ok := v["target_availability_zone"].(string); ok && zone != "" {
		res.TargetAvailabilityZone = resourcemover.TargetAvailabilityZone(zone)
	}

	return res
}

func flattenArmMoveResourceVirtualMachineResourceSettings(input *resourcemover.VirtualMachineResourceSettings) []interface{} {
	if input == nil {
	...
	if input.TargetAvailabilitySetID != nil {
		targetAvailabilitySetId = *input.TargetAvailabilitySetID
	}
	var targetAvailabilityZone resourcemover.TargetAvailabilityZone
	if input.TargetAvailabilityZone != "" {
		targetAvailabilityZone = input.TargetAvailabilityZone
	}
	var targetVmSize string
	if input.TargetVMSize != nil {
