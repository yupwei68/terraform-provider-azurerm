...
func resourceArmAdvisorSuppressionCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	recommendationId, _ := parse.AdvisorRecommendationID(d.Get("recommendation_id").(string))

	id := parse.NewAdvisorSuppressionID(recommendationId.ResourceUri, recommendationId.Name, name).ID("")

	if d.IsNewResource() {
		existing, err := client.Get(ctx, recommendationId.ResourceUri, recommendationId.Name, name)
		if err != nil {
		...
		if !utils.ResponseWasNotFound(existing.Response) {
			return tf.ImportAsExistsError("azurerm_advisor_suppression", id)
		}
	}
	...
	if _, err := client.Create(ctx, recommendationId.ResourceUri, recommendationId.Name, name, props); err != nil {
		return fmt.Errorf("failure in creating Advisor Suppressions %q: %+v", name, err)
	}

	if _, err := client.Get(ctx, recommendationId.ResourceUri, recommendationId.Name, name); err != nil {
		return fmt.Errorf("retrieving Advisor Suppressions %q (Recommendation Name %q, Resource Uri %q): %+v", name, recommendationId.Name, recommendationId.ResourceUri, err)
	}
...
func resourceArmAdvisorSuppressionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("retrieving Advisor Suppressions %q (Recommendation Name %q, Resource Uri %q): %+v", id.Name, id.RecommendationName, id.ResourceUri, err)
	}
...
func resourceArmAdvisorSuppressionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err = client.Delete(ctx, id.ResourceUri, id.RecommendationName, id.Name); err != nil {
		return fmt.Errorf("deleting Advisor Suppressions %q (Recommendation Name %q, Resource Uri %q): %+v", id.Name, id.RecommendationName, id.ResourceUri, err)
	}
